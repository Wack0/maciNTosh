...
static PVOID PciPhysToVirt(ULONG Addr) {
	ULONG Area = Addr & 0xF0000000;
	ULONG Offset = Addr & ~0xF0000000;
	// This shouldn't be used for an address in main memory, only the PCI space.
	
	// 0x80000000 range always mapped to VA 0x60000000 by BAT
	if (Area == 0x80000000) return (PVOID) (Offset | 0x60000000);
	// Any other area in the PCI physical range is mapped 1:1 by page tables.
	return (PVOID)Addr;
}
...
//---------------------------------------------------------------------------------
	...
	if (Desc->BootImgBase != 0) {
		PUCHAR BootImages = ArcMemAllocTemp(Desc->BootImgSize + Desc->PtdrSize + Desc->WikiSize);
		memcpy(BootImages, (PVOID)(Desc->BootImgBase + 0x80000000u), Desc->BootImgSize + Desc->PtdrSize + Desc->WikiSize);
		void ArcFsInitRepart(PVOID Buffer, ULONG BootImgSize, ULONG PtdrSize, ULONG WikiSize);
		ArcFsInitRepart(BootImages, Desc->BootImgSize, Desc->PtdrSize, Desc->WikiSize);
	}
	...
	macio_ide_init((ULONG) PciPhysToVirt(Desc->MacIoStart), 3);
	if (Desc->MioAta6Start[0] != 0) macio_ide_init((ULONG)PciPhysToVirt(Desc->MioAta6Start[0]), 1);
	if (Desc->MioAta6Start[1] != 0) macio_ide_init((ULONG)PciPhysToVirt(Desc->MioAta6Start[1]), 1);
	...
	ULONG PciIntIndex = 0;
	for (ULONG i = 0; i < 32; i += 2, PciIntIndex++) {
		ULONG PciIntValue = Desc->Pci1SlotInterrupts[PciIntIndex];
		s_RuntimePciInt[i + 0].Slot = (UCHAR)(PciIntValue >> 24);
		s_RuntimePciInt[i + 0].Vector = (UCHAR)(PciIntValue >> 16);
		s_RuntimePciInt[i + 1].Slot = (UCHAR)(PciIntValue >> 8);
		s_RuntimePciInt[i + 1].Vector = (UCHAR)(PciIntValue >> 0);
	}
	s_RuntimePointers[RUNTIME_PCI_INTERRUPTS].v = (ULONG)s_RuntimePciInt;
