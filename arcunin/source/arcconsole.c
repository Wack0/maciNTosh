...
void ArcConsoleInit(void* framebuffer, int xstart, int ystart, int xres, int yres, int stride)
{
	...
	g_framebuffer_phys = (ULONG)framebuffer;
	if ((g_framebuffer_phys & 0xf0000000) == 0x60000000) {
		g_framebuffer_phys += 0x20000000;
	}
...
int ArcConsoleWrite(const BYTE* ptr, size_t len)
{
	...
	while (*tmp != '\0' && i < len)
	{
		...
		if (con->cursor_row >= con->con_rows)
		{
			...
			for (ULONG destPtr = (ULONG)con->destbuffer; destPtr < (ULONG)con->destbuffer + con->con_stride * (FONT_YSIZE * FONT_YFACTOR + FONT_YGAP); srcPtr += 4, destPtr += 4) {
			...
			while (cnt--) {
				NativeWrite32(ptr, con->background);
				ptr++;
			}
			con->cursor_row--;
		}
