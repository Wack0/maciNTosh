...
#define IDEREG_CONTROL	0x16 // not 0x08. qemu accepts that, real hardware does NOT.
...
struct ide_drive {
	...
	char        atapi_ready; // drive is ready for atapi-cd
...
typedef struct ide_channel IDE_CHANNEL, *PIDE_CHANNEL;
typedef struct ide_drive IDE_DRIVE, *PIDE_DRIVE;
 ...
 */
static ARC_FORCEINLINE ULONG ob_ide_max_transfer(PIDE_DRIVE drive)
{
	return drive->max_sectors * drive->bs;
}

static ARC_FORCEINLINE ULONG ob_ide_block_size(PIDE_DRIVE drive) {
	return drive->bs;
}
