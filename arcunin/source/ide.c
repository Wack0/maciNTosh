...
static int ob_ide_atapi_test_unit_ready(struct ide_drive* drive) {
	struct atapi_command* cmd = &drive->channel->atapi_cmd;
	memset(cmd, 0, sizeof(*cmd));
	cmd->cdb[0] = ATAPI_TUR;
...
ob_ide_atapi_drive_ready(struct ide_drive *drive)
{
	 ...
	 */
	if (ob_ide_atapi_test_unit_ready(drive) != 0) return 1;
                  ...
                  unsigned char *buf, unsigned int sectors)
{
	...
	if (!drive->atapi_ready) {
		drive->atapi_ready = (ob_ide_atapi_drive_ready(drive) == 0);
		if (!drive->atapi_ready) return 1;
	}
	else {
		if (ob_ide_atapi_test_unit_ready(drive) != 0) return 1;
	}
...
ob_ide_identify_drive(struct ide_drive *drive)
{
	...
	if (drive->type == ide_type_atapi) {
		...
		if (drive->channel->has_lba48 && (id.command_set_2 & 0x0400) && (id.cfs_enable_2 & 0x0400)) {
...
bool ob_ide_eject(PIDE_DRIVE drive) {
	if (drive->type != ide_type_atapi) return false;
	...
	/*
	 * START/STOP UNIT with eject
	 */
	cmd->cdb[0] = ATAPI_START_STOP_UNIT;
	cmd->cdb[4] = 2;


	return (ob_ide_atapi_packet(drive, cmd) == 0);
}
...
PIDE_DRIVE ob_ide_open(int channel, int unit)
{
	...
	if (drive->type != ide_type_ata) {
		...
		drive->atapi_ready = (ob_ide_atapi_drive_ready(drive) == 0);
		if (!drive->atapi_ready) drive->atapi_ready = (ob_ide_atapi_drive_ready(drive) == 0);
	}
...
#define MACIO_IDE_OFFSET	0x0001F000
#define MACIO2_IDE_OFFSET   0x00002000
...
int macio_ide_init(uint32_t addr, int nb_channels)
{
	...
	// If we already have some IDE channels then this is a secondary controller with 1 channel + dbdma
	bool is_secondary_controller = (s_channels_head != NULL);
	if (is_secondary_controller) {
		nb_channels = 1;

		// ensure the controller is enabled
		MmioWrite32L((PVOID)addr, 7);
	}
	int last_channel = 0;
	while (*last != NULL) {
		chan = *last;
		last = &chan->next;
		if (chan->channel >= last_channel) last_channel = chan->channel + 1;
	}
	for (i = 0; i < nb_channels; i++) {
		...
		if (is_secondary_controller) chan->mmio = addr + MACIO2_IDE_OFFSET;
		IDE_DPRINTF("Channel %d, virtual address %08x", last_channel + i, chan->mmio);
		chan->channel = last_channel + i;
		chan->has_lba48 = is_secondary_controller; // ATA-6 controllers have LBA48, others do not
