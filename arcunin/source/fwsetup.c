...
static void EjecterPrintDiskEntry(ULONG Cd, ULONG ExitIndex) {
	if (Cd == ExitIndex) printf("Cancel");
	else {
		char CdVar[6];
		snprintf(CdVar, sizeof(CdVar), "cd%02d:", Cd);
		printf("%s - %s", Cd == 0 ? "cd:" : CdVar, ArcEnvGetDevice(CdVar));
	}
}
...
static bool ArcFwCheckInstallMedia(void) {
	if (ARC_FAIL(ArcFsRepartFilesOnDisk())) return false;
	return true;
}
	...
	while (1) {
		...
		if (DefaultChoice == PARTITION_MENU_CHOICE_FINISH) {
			...
			if (ARC_SUCCESS(Status)) {
				Status = ArcFsRepartitionDisk(FileId.v, NtPartitionSize, s_MacPartitions, CountMacPartitions, &DataWritten);
...
static void ArcFwPartitionerUpdate(ULONG Hd) {
	...
	ARC_STATUS Status = Api->OpenRoutine(HdDevice, ArcOpenReadWrite, &FileId);
	if (ARC_SUCCESS(Status)) {
		Status = ArcFsUpdateBootPartition(FileId.v);
		Api->CloseRoutine(FileId.v);
	}
...
static void ArcFwPartitioner(ULONG SetupMenuChoice) {
	...
	if (!ArcFwCheckInstallMedia()) {
		printf(" Installation files were not loaded by the bootloader.\r\n");
		printf(" Press any key to continue...\r\n");
		IOSKBD_ReadChar();
		return;
	}
...
static void ArcFwEjectOpticalDrive(void) {
	ULONG CdCount;
	ArcDiskGetCounts(NULL, &CdCount);
	ULONG MenuCount = CdCount + 1;
	...
	while (1) {
		...
		for (ULONG i = 0; i < MenuCount; i++) {
			...
			EjecterPrintDiskEntry(i, ExitIndex);
			ArcSetScreenAttributes(true, false, false);
		}
		...
		do {
			if (IOSKBD_CharAvailable()) {
				...
				case 0x9b:
					Character = IOSKBD_ReadChar();
					ArcSetPosition(DefaultChoice + 4, 5);
					EjecterPrintDiskEntry(DefaultChoice, ExitIndex);

					switch (Character) {
					case 'A': // Up arrow
					case 'D': // Left arrow
						if (DefaultChoice == 0) DefaultChoice = CdCount;
						DefaultChoice--;
						break;
					case 'B': // Down arrow
					case 'C': // Right arrow
		...
		// open hd
		char CdVar[6];
		snprintf(CdVar, sizeof(CdVar), "cd%02d:", DefaultChoice);

		U32LE FileId;
		PCHAR CdDevice = ArcEnvGetDevice(CdVar);
		ARC_STATUS Status = Api->OpenRoutine(CdDevice, ArcOpenReadOnly, &FileId);
		if (ARC_SUCCESS(Status)) {
			Status = ArcDiskTryEject(FileId.v);
			Api->CloseRoutine(FileId.v);
		}

		if (ARC_FAIL(Status)) {
			printf("Failed to eject %s: %s\r\n", CdVar, ArcGetErrorString(Status));
			printf(" Press any key to continue...\r\n");
			IOSKBD_ReadChar();
			return;
		}
...
void ArcFwSetup(void) {
	while (1) {
		...
		if (DefaultChoice == SETUP_MENU_CHOICE_EJECTODD) {
			ArcFwEjectOpticalDrive();
			return;
		}
